...
isAutobiographical(1210) ➞ true // one 0, two 1's, one 2, zero 3's
isAutobiographical(638) ➞ false
isAutobiographical(0) ➞ false // claims no zeroes, however zero is the first digit
...
function isAutobiographical(n) {
  ...
  const filterFunc = (arr, i) => {
    return arr.filter((elem) => elem === i.toString()).length;
  };

  const result = [];

  arr.forEach((elem, index) => {
    result.push(filterFunc(arr, index) == parseInt(elem));
  });
